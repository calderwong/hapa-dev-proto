...
const NAV_ITEMS: NavItemConfig[] = [
    ...
    { path: '/prototypes', label: 'Prototypes', icon: 'build' },
...
const Layout: React.FC = () => {
    ...
    const [hopeBurstKey, setHopeBurstKey] = React.useState(0);
    const bottomDockRef = React.useRef<HTMLDivElement | null>(null);
    ...
    const triggerHope = React.useCallback(() => {
        setHopeBurstKey(Date.now());
    }, []);
    ...
    React.useEffect(() => {
        ...
        const publish = () => {
            ...
            window.dispatchEvent(
                new CustomEvent('hapa.bottomDock.rect', {
                    detail: { rect: { left: r.left, top: r.top, width: r.width, height: r.height } },
                })
            );
        };
